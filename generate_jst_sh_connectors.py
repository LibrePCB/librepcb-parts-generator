...
def footprint_add_support_pads(footprint: Footprint, connector: Connector, spec: FootprintSpecification) -> None:
    for i in range(2):
        footprint.add_pad(
            FootprintPad(
                ...
                position=Position(
                    spec.support_pad_first_x_center + i * spec.support_pad_distance_x(connector.circuits),
...
def footprint_add_pads(
    footprint: Footprint, connector: Connector, spec: FootprintSpecification, reverse_pad_order: bool
) -> None:
    for i in range(connector.circuits):
        ...
        footprint.add_pad(
            FootprintPad(
                ...
                shape=Shape.ROUNDED_RECT,
                position=Position(
                    spec.pad_first_x_center + i * spec.pad_distance_mid_to_mid_x, spec.pad_first_y_center
                ),
                rotation=Rotation(0),
...
def footprint_add_header(footprint: Footprint, connector: Connector, spec: FootprintSpecification) -> None:
...
def footprint_add_text(footprint: Footprint, connector: Connector, spec: FootprintSpecification, rotation: int) -> None:
...
def footprint_add_leads(footprint: Footprint, connector: Connector, spec: FootprintSpecification) -> None:
    for i in range(connector.circuits):
...
def footprint_add_outline(footprint: Footprint, connector: Connector, spec: FootprintSpecification) -> None:
    outline_polygon = Polygon(
        ...
        vertices=[
            ...
            vertex(
                spec.pad_first_x_center - (spec.pad_width / 2), spec.header_y + spec.lead_height + spec.header_height
            ),
...
def footprint_add_courtyard(footprint: Footprint, connector: Connector, spec: FootprintSpecification) -> None:
    courtyard_polygon = Polygon(
        ...
        vertices=[
            ...
            # right bottom
            vertex(
                spec.support_pad_distance_x(connector.circuits) + spec.support_pad_width + courtyard_excess,
                0 - courtyard_excess,
            ),
            # right top
            vertex(
...
def footprint_add_legend(footprint: Footprint, connector: Connector, spec: FootprintSpecification) -> None:
...
def generate_footprint(
    connector: Connector, spec: FootprintSpecification, description: str, reverse_pad_order: bool, rotation: int
) -> Footprint:
    footprint = Footprint(
