...
def generate_pkg(
    ...
    for config in configs:
        ...
        def add_footprint_variant(key: str, name: str, pad_size: Tuple[float, float]) -> None:
            ...
            for p in range(1, pin_count // 2 + 1):
                ...
                footprint.add_pad(
                    FootprintPad(
                        ...
                        [PadHole(uuid_pads[p - 1], DrillDiameter(drill_diameter), [Vertex(Position(0, 0), Angle(0))])],
                    )
                )
            for p in range(1, pin_count // 2 + 1):
                # Up on the right
            ...
            notch_dx = dx / 4
            dy1 = get_y(1, pin_count // 2, pitch, False) + pad_size[1] / 2 + line_width / 2 + silkscreen_offset
