...
def generate_pkg(config: Config) -> None:
    ...
    for i in range(1, config.pin_count + 1):
        ...
        footprint.add_polygon(
            Polygon(
                ...
                vertices=[
                    Vertex(
                        Position(coords.x - config.lead_width / 2 * sign, coords.y + config.lead_width / 2), Angle(0)
                    ),
                    Vertex(
                        Position(coords.x - config.lead_width / 2 * sign, coords.y - config.lead_width / 2), Angle(0)
                    ),
    ...
    footprint.add_polygon(
        Polygon(
            ...
            vertices=[
                ...
                Vertex(
                    Position(-x_inside_body + triangle_offset, y_inside_body - triangle_offset - triangle_size),
                    Angle(0),
                ),
                Vertex(
                    Position(
    ...
    def _create_outline(polygon_uuid: str, polygon_layer: str, polygon_offset: float, around_pads: bool) -> Polygon:
