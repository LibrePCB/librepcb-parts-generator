...
def generate_3d_model_generic(
    ...
    if model_type == 'female':
        ...
        insulator = cq.Workplane('XY').box(spacing, spacing + a_little, standoff_height, centered=(True, True, False))
    ...
    for i in range(min_pads, max_pads + 1, rows):
        ...
        if kind == KIND_HEADER:
            ...
            for p in range(1, i + 1):
                ...
                polygon = Polygon(
                    uuid_decoration, Layer('sym_outlines'), Width(line_width * 0.75), Fill(False), GrabArea(False)
                )
