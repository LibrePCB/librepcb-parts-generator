...
def generate(
    ...
    if pin1_indicator:
        body = body.workplane(origin=(dot_center[0], dot_center[1]), offset=(body_height / 2) - dot_depth).cylinder(
            5, dot_diameter / 2, centered=(True, True, False), combine='cut'
        )
    assembly.add_body(body, 'body', StepColor.IC_BODY)

    if pin1_indicator:
        dot = cq.Workplane('XY', origin=dot_center).cylinder(0.05, dot_diameter / 2, centered=(True, True, False))
    ...
    for x_sgn, rotation, leads in [(-1, 0, leads_left), (1, 180, leads_right)]:
        for lead_name, lead_y, lead_width in leads:
            if lead_width not in leads_by_width:
                ...
                leads_by_width[lead_width] = cq.Workplane('ZY').rect(lead_height, lead_width).sweep(lead_path)
            assembly.add_body(
