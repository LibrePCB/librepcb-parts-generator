...
def generate_leg_path(plane: str, length: float, delta: float) -> cq.Workplane:
    straight_length = 1
    return (
        cq.Workplane(plane).vLine(length - straight_length - abs(delta)).line(-delta, abs(delta)).vLine(straight_length)
    )
...
def generate(
    ...
    body = (
        cq.Workplane('XY', origin=(0, 0, body_standoff))
    ...
    leg_straight = cq.Workplane('XY', origin=(0, 0, leg_z)).box(
        leg_width, leg_width, leg_length, centered=(True, True, False)
    )
    if pitch_x == 1.27:
        leg_1 = leg_straight
        leg_3 = leg_straight
    ...
    assembly.add_body(leg_1, 'leg-1', StepColor.LEAD_SMT, location=cq.Location((-1.27, 0, 0)))
    assembly.add_body(leg_2, 'leg-2', StepColor.LEAD_SMT, location=cq.Location((0, 0, 0)))
    assembly.add_body(leg_3, 'leg-3', StepColor.LEAD_SMT, location=cq.Location((1.27, 0, 0)))
