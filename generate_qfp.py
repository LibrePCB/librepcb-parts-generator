    ...
    for config in configs:
        ...
        def add_footprint_variant(
            ...
            for quadrant in [1, 2, 3, 4]:
                ...
                footprint.add_polygon(
                    Polygon(
                        ...
                        fill=Fill(False),
                        grab_area=GrabArea(False),
                        vertices=[Vertex(Position(sign_x * x, sign_y * y), Angle(0)) for (x, y) in vertices],
                    )
                )
            ...
            def _create_outline_vertices(offset: float = 0, around_pads: bool = False) -> List[Vertex]:
                ...
                return [Vertex(Position(x + sign(x) * offset, y + sign(y) * offset), Angle(0)) for (x, y) in vertices]
